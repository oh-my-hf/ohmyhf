import type { RepoKind, RepoOpenTarget } from '@oh-my-huggingface/shared'
...
export function repoHubUrl(
  ...
  const base = (hubEndpoint ?? 'https://huggingface.co').replace(/\/$/, '')
  return `${base}/${HUB_PREFIX[kind]}${repoId}`
}
